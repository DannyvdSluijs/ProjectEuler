...
class Problem067
{
    ...
    public function __invoke(): string
    {
        ...
        while ($queue->valid()) {
            ...
            printf("Depth %d (avg: %f, sum: %d) to queue with prio: %f\r\n", $nextLayer, array_sum($leftArm) / ($nextLayer + 1), array_sum($leftArm), array_sum($leftArm) / ($nextLayer + 1));
            $queue->insert([
                'path' => $rightArm,
                'pos' => $state['pos'] + 1,
                'layer' => $nextLayer
            ], array_sum($rightArm) / ($nextLayer + 1) );
            printf("Depth %d (avg: %f, sum: %d) to queue with prio: %f\r\n", $nextLayer, array_sum($rightArm) / ($nextLayer + 1), array_sum($rightArm), array_sum($rightArm) / ($nextLayer + 1));
        }
